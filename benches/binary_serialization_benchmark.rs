...
// =============================================================================
// Representative serde-serializable data — mirrors a resolved DixScript payload
// =============================================================================
...
fn medium_payload() -> SmallPayload {
    ...
    let enemies = (0..20)
        .map(|i| make_enemy(&format!("{}_{}", names[i % names.len()], i), 50 * ((i % 8) + 1) as i32, (50 * ((i % 8) + 1) / 5) as i32))
        .collect();
    SmallPayload {
        app_version: "1.0.0".to_string(),
        enemies,
...
// =============================================================================
// DixScript AST sources for custom format tests
// =============================================================================
...
fn parse_ast(source: &str) -> DixScript {
...
fn pack_dixscript(ast: &DixScript) -> Vec<u8> {
    let result = BinaryPacker::new().pack(ast);
    if result.is_success {
        result.binary_data
    } else {
        panic!("BinaryPacker failed: {:?}", result.errors)
    }
}
...
fn print_comparison_report() {
    let ast = parse_ast(SMALL_MDIX);
    let payload_small = small_payload();
    let payload_medium = medium_payload();

    // Serialize with every format
    let custom_small = pack_dixscript(&ast);
    ...
    let entries: &[(&str, &[u8], &[u8])] = &[
        ("DixScript Custom", &custom_small, &custom_small), // no medium variant (AST fixed)
        ("Bincode",          &bincode_small, &bincode_medium),
        ("Postcard",         &postcard_small, &postcard_medium),
        ("MessagePack",      &msgpack_small, &msgpack_medium),
    ];
    ...
    for (label, small, medium) in entries {
    ...
    println!("  {:<20} {:>9} {:>10} {:>10} {:>10}", "Format", "Raw", "Gzip", "Bzip2", "LZMA");
    println!("  {}", "─".repeat(63));
    for (label, small, _) in entries {
        let gz = compress_gzip(small);
        let bz = compress_bzip2(small);
        let xz = compress_lzma(small);
        println!("  {:<20} {:>9} {:>10} {:>10} {:>10}", label, small.len(), gz.len(), bz.len(), xz.len());
    }
    println!();
}
...
fn bench_serialization_speed(c: &mut Criterion) {
    ...
    let ast_small = parse_ast(SMALL_MDIX);
    let payload_small  = small_payload();
    let payload_medium = medium_payload();

    // Custom binary format (pack from pre-parsed AST)
...
fn bench_compression_pipeline(c: &mut Criterion) {
    ...
    // Pre-pack once so we time only compression in the dixscript+compress cases
    let custom_bytes   = pack_dixscript(&ast_small);
    ...
    let cases: &[(&str, &[u8])] = &[
        ("dixscript_custom", &custom_bytes),
        ("bincode",          &bincode_bytes),
