...
pub enum ImportsResolutionErrorType {
    ...
    AliasConflict = 7,
    /// The import path does not have .dixscript extension
    InvalidExtension = 8,
    ...
    fn generate_suggestion(
        ...
        match error_type {
            ...
            ImportsResolutionErrorType::InvalidPath => {
                "Import paths must use forward slashes (/) and end with .dixscript extension. \
                 Use relative paths (e.g., '../shared/utils.dixscript') instead of absolute paths.".to_string()
            }
            ...
            ImportsResolutionErrorType::InvalidExtension => {
                "Import path must end with .dixscript extension. DixScript only supports importing .dixscript files.".to_string()
            }
