...
mod tests {
    ...
    fn type_annotations_complete() {
        ...
        for t in &["<int>","<long>","<float>","<double>","<string>","<bool>","<array>",
                   "<tuple>","<object>","<hex>","<blob>","<regex>","<date>","<timestamp>","<enum>","<any>"] {
    ...
    #[test]
    fn typed_array_maps_to_array_dix_type() {
        use dixscript::Compiler::AST::ElemType;
        let dt = DataType::TypedArray(ElemType::Int);
        assert_eq!(completion_dt_to_dix(dt), Some(DixType::Array));
    }

    #[test]
    fn typed_tuple_maps_to_tuple_dix_type() {
        use dixscript::Compiler::AST::ElemType;
        let dt = DataType::TypedTuple([Some(ElemType::Int), Some(ElemType::Bool), None, None, None, None]);
        assert_eq!(completion_dt_to_dix(dt), Some(DixType::Tuple));
    }
