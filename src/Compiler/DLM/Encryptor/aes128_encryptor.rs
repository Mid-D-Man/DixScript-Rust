//! AES-128-GCM encryption implementation (MEDIUM security, faster)
...
impl IEncryptor for Aes128Encryptor {
    ...
    fn set_password(&mut self, password: &str) -> EncryptorResult<()> {
        ...
        self.key = kdf.derive_key(password, 16)?; // 128 bits = 16 bytes
        self.iv = Self::generate_iv();
        self.kdf = Some(kdf);
