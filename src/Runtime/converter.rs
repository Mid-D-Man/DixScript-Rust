...
impl DixConverter {
        ...
        match value {
            DixValue::Object(obj) => {
                ...
                if !properties.is_empty() {
                    entries.push(DataEntry::TableProperty { path, properties });
                }
    ...
    fn convert_dix_value_to_ast_value(&self, value: &DixValue) -> Result<Value, String> {
        match value {
            DixValue::Null => Ok(Value::Null),
            DixValue::Bool(b) => Ok(Value::Boolean(*b)),
            DixValue::Int(i) => Ok(Value::Integer(*i)),
            DixValue::Float(f) => Ok(Value::Float(*f)),
            DixValue::Double(d) => Ok(Value::Double(*d)),
            DixValue::String(s) => Ok(Value::StringLiteral(s.clone())),
            DixValue::Date(d) => Ok(Value::Date(d.clone())),
            DixValue::Timestamp(t) => Ok(Value::Timestamp(t.clone())),
            DixValue::HexColor(c) => Ok(Value::HexColor(c.clone())),
            DixValue::Blob(b) => Ok(Value::PrefixedConstructor {
                prefix: "b".to_string(),
                arguments: vec![Value::StringLiteral(b.clone())],
            }),
            DixValue::Regex(r) => Ok(Value::PrefixedConstructor {
                prefix: "r".to_string(),
                arguments: vec![Value::StringLiteral(r.clone())],
            }),
            DixValue::Array(arr) => {
                let items: Result<Vec<Value>, String> = arr
                    .iter()
            ...
            DixValue::Object(obj) => {
                ...
                Ok(Value::ObjectLiteral(properties))
            }
    ...
    fn flatten_entry(
        ...
        match entry {
            ...
            DataEntry::ObjectProperty { name, object, .. } => {
                ...
                if let Value::ObjectLiteral(props) = object {
