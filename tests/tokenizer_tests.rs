...
fn test_single_identifier() {
    ...
    assert!(matches!(&result.tokens[0].token_type, TokenType::Identifier(id) if id == "hello"));
}
...
fn test_multiple_identifiers() {
    ...
    assert!(matches!(&result.tokens[0].token_type, TokenType::Identifier(id) if id == "hello"));
    assert!(matches!(&result.tokens[1].token_type, TokenType::Identifier(id) if id == "world"));
    assert!(matches!(&result.tokens[2].token_type, TokenType::Identifier(id) if id == "test"));
}
...
fn test_double_quoted_strings() {
    ...
    assert!(matches!(&result.tokens[0].token_type, TokenType::String(s) if s == "Hello World"));
    assert!(matches!(&result.tokens[1].token_type, TokenType::String(s) if s == "test"));
    assert!(matches!(&result.tokens[2].token_type, TokenType::String(s) if s == ""));
}
...
fn test_single_quoted_strings() {
    ...
    assert!(matches!(&result.tokens[0].token_type, TokenType::StringSingle(s) if s == "Hello"));
    assert!(matches!(&result.tokens[1].token_type, TokenType::StringSingle(s) if s == "test"));
    assert!(matches!(&result.tokens[2].token_type, TokenType::StringSingle(s) if s == ""));
}
...
fn test_dates() {
    ...
    assert!(matches!(&result.tokens[0].token_type, TokenType::Date(d) if d == "2025-01-15"));
    assert!(matches!(&result.tokens[1].token_type, TokenType::Date(d) if d == "2024-02-29"));
    assert!(matches!(&result.tokens[2].token_type, TokenType::Date(d) if d == "2025-12-31"));
}
...
fn test_hex_colors() {
    ...
    assert!(matches!(&result.tokens[0].token_type, TokenType::HexColor(c) if c == "#F00"));
    assert!(matches!(&result.tokens[1].token_type, TokenType::HexColor(c) if c == "#FF0000"));
    assert!(matches!(&result.tokens[2].token_type, TokenType::HexColor(c) if c == "#F00F"));
    assert!(matches!(&result.tokens[3].token_type, TokenType::HexColor(c) if c == "#FF0000FF"));
    assert!(matches!(&result.tokens[4].token_type, TokenType::HexColor(c) if c == "#80FFFFFF"));
}
...
fn test_static_call_detection() {
    ...
    assert_eq!(result.static_calls[0].object_name, "DateTime");
    assert_eq!(result.static_calls[0].method_name, "now");
}
...
fn test_all_datatypes_mdix_file() {
    let file_content = std::fs::read_to_string("DixScriptConsoleApp/all_datatypes_test.mdix")
...
fn test_large_input_performance() {
    ...
    let result = tokenize_input(&large_input);
    let duration = start.elapsed();

    println!("Tokenized {} tokens in {:?}", result.tokens.len(), duration);

    // Should complete in reasonable time
    assert!(duration.as_secs() < 1);
    assert!(result.tokens.len() > 3000); // Each statement has 4+ tokens
}